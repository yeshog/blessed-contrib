...
function yhioeProcessDeviceSnapshot(snapshot) {
    yhioeLogMsg("Snapshot " + snapshot);
    let snap = getObjectIfValidJson(snapshot);
    if (!snap) {
        yhioeLogMsg("Snapshot not yet available");
        return;
    }
    let dvc_snap = snap[0];
...
function yhioeModuleSM(text, txtbox, cb, cbarg) {
    ...
    switch (yhioeLiveData.state) {
        ...
        case YH_STATE_LOGGED_IN:
            ...
            if (text && text.length) {
                yhioeLogMsg(text);
                /* localhost or current host can let you
                access stuff simply over a cookie
                 */
                if (yhioeLiveData.connection.port === '4433') {
