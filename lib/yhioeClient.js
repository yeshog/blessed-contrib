...
function secStrDataOp(data, op, pathIfAny, tagPathIfAny) {
    ...
    var filename = pathIfAny? pathIfAny : path.join(cwd, ".secData");
    ...
    if (op == 'enc') {
        ...
        try {
            fs.writeFileSync(filename, encrypted.toString('base64'));
        ...
        var buff = null;
        try {
            let b64data = fs.readFileSync(filename).toString();
            let tagData = data? data : fs.readFileSync(secDataTag).toString();
        ...
        case YH_STATE_LOGGED_IN:
            ...
            if (text && text.length) {
                authenticateAndFetchWithParams(
                        ...
                        yhioeLiveData.connection.port,
                        '/c',
                        yhioeLiveData.credentials, {query: text},
                        cb, cbarg);
            }
            break;
        default:
...
function yhioeClientTest() {
    // setInterval(() => {
    //     // yhioeRunAcctStats();
    //     // console.log(JSON.stringify(yhioeLiveData.yhAcctStats,
    //     //     null, 2))
    //     let counts = yhioeGetUserAndGlobalCounts();
    //     console.log(JSON.stringify(counts));
    // }, 16000);

    // authenticateAndFetch('127.0.0.1',
    //     44433, '/n/g',{
